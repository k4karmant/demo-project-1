...
public class EUserDao {
	...
	public void insertUser(EUserBean userBean) {
		// query ->
		stmt.update("insert into user(firstName,email,password,profilePicpath) values (?,?,?,?) ", userBean.getFirstName(),
	...
	public EUserBean authenticate(String email, String password) {
		...
		try {
			EUserBean userBean = stmt.queryForObject("select * from user where email = ? and password = ?",
